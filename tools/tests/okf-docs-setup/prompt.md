...
Copy the verbatim asset files exactly as shipped: do NOT modify `scripts/validate-docs.mjs` or `scripts/validate-docs.test.mjs` in any way — leave them byte-identical to the skill's assets. After copying and substituting you MAY run the docs validator once to confirm it exits 0 with zero hard ERRORS, but do NOT chase or fix warnings and do NOT edit any installed file. Do NOT wire anything into CLAUDE.md or AGENTS.md — skip the project-memory step entirely.
